...
public class AddressBookGUI extends Application {
    ...
    private void showEditContactDialog() {
        ...
        result.ifPresent(selectedContactName -> {
            ...
            if (index >= 0) {
                Contact selectedContact = addressBook.getContacts().get(index);
                ...
                editResult.ifPresent(contactDetails -> {
                    // Parse the updated contact details (assuming they are comma-separated)
                    String[] detailsArray = contactDetails.split(",");
